...
import com.example.androidscaffolding.databinding.FragmentLogin2Binding
...
class Login2Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.signupOKBtn.setOnClickListener {
            ...
            activity?.supportFragmentManager?.beginTransaction()?.replace(R.id.tabContent,signup2Fragment)?.commit()
        }
                ...
                MyApplication.auth.signInWithCredential(credential)
                    ...
                    .addOnCompleteListener(Activity()) {
                        ...
                        if(task.isSuccessful){
                            ...
                            val pref = activity?.getSharedPreferences("loginInfo",
                            ...
                            activity?.supportFragmentManager?.beginTransaction()?.replace(R.id.tabContent,boardsFragment)?.commit()
//                            finish()
...
//                chageVisi("logout")
        ...
        binding.googleAuthBtn.setOnClickListener {
            ...
            val gso = GoogleSignInOptions
                ...
                .build()
            // 구글 인증 화면으로 이동하는 코드
            val signInIntent = activity?.let { it1 -> GoogleSignIn.getClient(it1,gso).signInIntent }
            // 후처리 함수 동작 연결시키기.
        ...
        binding.googleLogoutBtn.setOnClickListener {
            ...
            val pref = activity?.getSharedPreferences("loginInfo", Context.MODE_PRIVATE)
            val editor = pref?.edit()
            editor?.clear()
            editor?.commit()
        }
